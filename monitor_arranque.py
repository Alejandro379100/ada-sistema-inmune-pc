...
def verificar_arranque() -> str:
    ...
    nuevos = {
        ...
        and not any(ig in k.lower() or ig in v.lower() for ig in IGNORAR)
    }
...
def analizar_con_groq(preguntar_groq_fn) -> str:
    ...
    relevantes = {
        k: v for k, v in actual.items()
        if not any(ig in k.lower() or ig in v.lower() for ig in IGNORAR)
    }
    ...
    lista = "\n".join([f"- {k}: {v[:80]}" for k, v in list(relevantes.items())[:15]])
    prompt = (
